...
function createOrJoin() {
    ...
    do
		...
		then
			...
			${globalDir}/ibft/join_network.sh --id 1101 --nn "node"${node_no} --pw "node"${node_no} --nt "y" --ip ${pCurrentIp} --mip ${pCurrentIp} --mport 22000 --r ${port_no} --w $((port_no + 1))  --t $((port_no + 2)) --dt $((port_no + 3)) --ws $((port_no + 4)) 
            cd "node"${node_no}
            ./node_start.sh
            cd ../
        else
            echo -e $CYAN"Configuring node${node_no}"$COLOR_END
            ${globalDir}/ibft/create_network.sh --id 1101 --nn "node"${node_no} --pw "node"${node_no} --nt "y" --ip ${pCurrentIp} --r ${port_no} --w $((port_no + 1))  --t $((port_no + 2)) --dt $((port_no + 3)) --ws $((port_no + 4))
            cd "node"${node_no}
            ./node_start.sh
            cd ../
		fi
